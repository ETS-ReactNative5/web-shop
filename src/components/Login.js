...
class Login extends React.Component {
  ...
  getSetting(){
    ...
    .then(response => {
      that.setState({
        ...
        AllowRegister: response.data.result ? response.data.result.AllowRegister : false,
        Theme: response.data.result ? response.data.result.Theme : "1"
      })
      if(response.data.result.System == "company"){
  ...
  GetShopList() {
    ...
    let SCallBack = function (response) {
      that.setState({
        shopId : response.data.result[0] ? response.data.result[0]._id : null
      })
    };
    ...
    if(!this.state.inputPassword && this.state.Step !=2){
      ...
      .then(response => {
        ...
        if(response.data.result=="no" ){
          ...
          if(this.state.System =="shop" || this.state.AllowRegister){
            this.setState({
              Step:2
            })
            this.Register();
                ...
                <div className={(this.props.noHeader) ? "card-signin2" : "card card-signin"} style={{paddingTop:20,paddingBottom:40}} >
                  <div className="card-body">
                   ...
                   {this.state.Step == "2" &&
                    ...
                    {this.state.Step == "2" &&
                    <div style={{textAlign:'center'}}>
                          <label className="YekanBakhFaBold">تا چند ثانیه دیگر کد تایید به شماره موبایل شما پیامک خواهد شد</label>
                    </div>
                   }
