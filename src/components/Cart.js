...
class Cart extends React.Component {
    constructor(props) {
        ...
        this.state = {
            GridData: [],
            Today:moment().locale('fa').format('jYYYY/jM/jD'),
            layout: 'list',
            lastPrice: 0,
            CartItemsGet: 0,
            CartNumber: 0,
        ...
        if (!this.state.AcceptAddress) {
            axios.post(this.state.url + 'getuserInformation', {
                ...
                .then(response => {
                    ...
                    let MaxChequeMounth = parseInt(this.state.ChequeInfo?.MaxChequeMounth)*30;
                    let ChequeCommission = parseInt(this.state.ChequeInfo?.ChequeCommission);
                    ...
                    let DateTelorance = 0;
                    let Commission = 0;
                    let SumChequeAmoutnt=0;
                    let fPrice = parseInt(this.state.lastPrice) + parseInt(this.state.paykAmount);
                    if(this.state.ActiveBank == "Cheque"){
                        for(let i=0 ; i < param?.ChequeList.length;i++){
                            DateTelorance = ((Date.parse(param.ChequeList[i].InChequeDate) - Date.parse(this.state.Today)) / (1000*60*60*24));
                            if(param.ChequeList[i].InChequeAmount) 
                                Commission += (Math.ceil(DateTelorance/30) * (ChequeCommission) * parseInt(param.ChequeList[i].InChequeAmount)) / 100;
                            if(param.ChequeList[i].InChequeAmount)    
                                SumChequeAmoutnt += parseInt(param.ChequeList[i].InChequeAmount);
                            if(DateTelorance > MaxChequeMounth){
                                this.setState({
                                    loading:0
                                })
                                this.toast.current.show({ severity: 'warn', summary: <div> تاریخ چک نمی تواند بیش از {MaxChequeMounth} روز از زمان خرید باشد </div>, life: 8000 });
                                return;
                            }
                        }
                        if(param.ChequeList && param.ChequeList.length > 0 && SumChequeAmoutnt != fPrice){
                            this.setState({
                                loading:0
                            })
                            this.toast.current.show({ severity: 'error', summary: <div> مبالغ تعیین شده برای چک ها با مبلغ سفارش متفاوت است  </div>, life: 8000 });
                            return;
                        }
                        if(param.ChequeList && param.ChequeList.length > 0){
                            this.setState({
                                loading:0
                            })
                            this.toast.current.show({ severity: 'warn', summary: <div>مبلغ {Math.round(Commission)} تومان به عنوان کارمزد خرید چکی به مبلغ نهایی اضافه شده که در اولین چک باید پرداخت شود </div>, life: 8000 });
                            param.ChequeList[0].InChequeAmount = parseInt(param.ChequeList[0].InChequeAmount) + Math.round(Commission);
                            fPrice+=Math.round(Commission); 
                            this.setState({
                                "InChequeAmount_0":param.ChequeList[0].InChequeAmount,
                                chequeEditDisable:true,
                                orgLastPrice:this.state.orgLastPrice,
                                lastPrice:fPrice
                            })
                        }
                    }
            ...
            return;
                ...
                if (this.state.lastPrice != 0) {
                    ...
                    let param = {
                        ...
                        Credit: (this.state.InRaymand && this.state.finalCreditReducer > 0) ? parseInt(this.state.finalCreditReducer.toString()+"0") : this.state.finalCreditReducer,
    ...
    setScore(){
        ...
        let param = {
            ...
            price:parseInt(this.state.lastPrice) + parseInt(this.state.paykAmount||0)
        };
                ...
                axios.post(this.state.url + 'getSettings', {
                    ...
                    .then(response => {
                        that.setState({
                            ...
                            OriginalActiveBank: response.data.result ? response.data.result.ActiveBank : "none",
                            TypeOfPayment: (response.data.result && response.data.result.ActiveBank=="none") ? "2" : this.state.TypeOfPayment,
                ...
                <div style={{ marginTop: 15 }}>
                    <div className="row" style={{ alignItems: 'center' }}>
                        ...
                        <div className="col-lg-3 col-md-3  col-12 YekanBakhFaMedium" style={{ textAlign: 'center' }}>
                            <Link target="_blank" to={`${process.env.PUBLIC_URL}/Products?id=` + (car.product_detail_id || car.product_id)} >
                                <img src={pic} style={{ height: "140px" }} name="pic3"  alt="" />
                            </Link>
                        </div>
                        ...
                        <div className="col-12 mb-3">
                            <div className="row" style={{ alignItems: 'center' }} >
                                ...
                                <div className="col-lg-2 col-12 YekanBakhFaMedium" style={{ textAlign: 'center' }}>
                                    <InputNumber value={car.number} inputStyle={{ borderRadius: 0, padding: 0, textAlign: 'center', fontSize: 20 }} mode="decimal" showButtons readOnly={true} onValueChange={(e) => { if (car.number == e.value) return; this.changeCart(e.value, car.product_detail_id || car.product_id, car.user_id) }} min={1} max={car.products[0].number} />
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Header2 />
                {!this.state.loading ?
                <div  className={this.state.Theme == "2" ? "B-container" : "A-container"} style={{ direction: 'rtl' }} >
                        ...
                        {this.state.GridData.length > 0 ?
                            <div>
                                <div className="col-12" style={{ textAlign: 'right', marginBottom: 50 }}>
                                        {this.state.InRaymand &&
                                        <p>
                                            <Link to={`${process.env.PUBLIC_URL}/user?Active=9`} style={{  fontSize: 18,fontStyle:'italic' }} className="YekanBakhFaMedium">درخواست وام خرید اقساطی</Link>
                                        </p>
                                        }
                                        <div >
                                            <p className="yekan" style={{ paddingTop: 20 }}>شیوه پرداخت</p>
                                        </div>
                                    {(this.state.OriginalActiveBank != 'none') ? 
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                            <RadioButton inputId="TypeOfPayment1" name="TypeOfPayment" value="1" onChange={(e) => { 
                                            if(this.state.TypeOfPayment == "4"){
                                                this.setState({ lastPrice: this.state.orgLastPrice||this.state.price, chequeEditDisable:false });
                                            }
                                            this.setState({ ActiveBank: this.state.OriginalActiveBank,AcceptAddress: false,StepNumber: 1 }); this.setState({ TypeOfPayment: e.value }); }} checked={this.state.TypeOfPayment === '1'} />
                                        ...
                                        <div style={{ display: 'flex' }}>
                                            <RadioButton inputId="TypeOfPayment2" name="TypeOfPayment" value="2" onChange={(e) => { 
                                            if(this.state.TypeOfPayment == "4"){
                                                this.setState({ lastPrice: this.state.orgLastPrice||this.state.price, chequeEditDisable:false });
                                            } this.setState({ ActiveBank: 'inPlace',AcceptAddress: false,StepNumber: 1 }); this.setState({ TypeOfPayment: e.value }); }} checked={this.state.TypeOfPayment === '2'} />
                                        ...
                                        {this.state.SaleByCheque &&
                                            <div style={{ display: 'flex' }}>
                                                <RadioButton inputId="TypeOfPayment3" name="TypeOfPayment" value="4" onChange={(e) => { this.setState({ ActiveBank: 'Cheque',AcceptAddress: false,StepNumber: 1 }); this.setState({ TypeOfPayment: e.value }); }} checked={this.state.TypeOfPayment === '4'} />
                                    ...
                                    <div>
                                        <div style={{ display: 'flex',marginTop:30 }}>
                                            <RadioButton inputId="TypeOfPayment2" name="TypeOfPayment" value="2" onChange={(e) => { if(this.state.TypeOfPayment == "4"){
                                                this.setState({ lastPrice: this.state.orgLastPrice||this.state.price, chequeEditDisable:false });
                                            }
                                            this.setState({ ActiveBank: 'inPlace' }); this.setState({ TypeOfPayment: e.value,AcceptAddress: false,StepNumber: 1 }); }} checked={this.state.TypeOfPayment === '2'} />
                                            <label htmlFor="TypeOfPayment2" className="p-checkbox-label yekan" style={{ marginRight: 20, color: 'rgb(185 185 185)' }}>پرداخت در محل</label>
                                        </div>
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        {this.state.SaleByCheque &&
                                    ...
                                    {this.state.TypeOfPayment == 4 &&
                                        <div>
                                            <p style={{color:'red',fontSize:16,marginTop:10}} className="irsans">سود کارمزد چک ها با نرخ {this.state.ChequeInfo.ChequeCommission} درصد محاسبه خواهد شد</p>
                                                ...
                                                return(
                                                    <div className="row" style={{alignItems:'baseline'}}>
                                                        ...
                                                        <DatePicker
                                                                ...
                                                                persianDigits={false}
                                                                disabled={this.state.chequeEditDisable}
                                                        ...
                                                        <div className="col-lg-2">
                                                        <div className="group">
                                                            <input className="form-control irsans" disabled={this.state.chequeEditDisable}  autoComplete="off" type="text" id={"InChequeAmount_" + index} name={"InChequeAmount_" + index} value={this.state["InChequeAmount_" + index]} onChange={(event) => { this.setState({ ["InChequeAmount_" + index]: event.target.value.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",") }) }} required="true" />
                                                            <label>مبلغ چک (تومان)</label>
                                                        </div>
                                                        </div>
                                                        <div className="col-lg-2">
                                                        <div className="group">
                                                            <input className="form-control irsans"    disabled={!this.state.chequeEditDisable}  autoComplete="off" type="text" id={"InChequeName_" + index} name={"InChequeName_" + index} value={this.state["chequeName_" + index]} onChange={(event) => { this.setState({ ["InChequeName_" + index]: event.target.value.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",") }) }} required="true" />
                                                            <label>صاحب حساب</label>
                                                        </div>
                                                        </div>
                                                        <div className="col-lg-2">
                                                            <div className="group">
                                                                <input className="form-control yekan" disabled={!this.state.chequeEditDisable} autoComplete="off" onChange={this.FileUpload} type="file" id={"InChequeFile_" + index} name={"InChequeFile_" + index} />
                    ...
                    {this.state.GridData.length > 0 &&
                        <div className="col-lg-4">
                            ...
                            <div className="card mt-md-0 mt-5" style={{ padding: 40, borderRadius: 20 }}>
                                ...
                                <p className="YekanBakhFaMedium" style={{ textAlign: "center", marginTop: 40, borderBottom: "1px solid #eee",paddingTop:20,paddingBottom:20 }}><span style={{ paddingLeft: 25 }}>مبلغ قابل پرداخت </span> <span style={{ color: '#a01212', fontSize: 25, marginTop: 50 }}> {this.state.lastPrice != "0" ? this.persianNumber((parseInt(this.state.paykAmount) + parseInt(this.state.lastPrice)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) + "  تومان" : "        "}
                                ...
                                {this.state.AcceptAddress ?
                                    <p className="YekanBakhFaMedium" style={{ textAlign: 'center' }} >
                                        ...
                                        <Link to={`${process.env.PUBLIC_URL}/user?Active=5`} style={{ textDecoration: 'none', fontSize: 16,fontStyle:'normal' }} className="YekanBakhFaMedium">برای ویرایش آدرس اینجا کلیک کنید</Link>
                                    </p>
                                    :
                                    <p>
                                ...
                                {(this.state.ActiveBank != "none" && this.state.ActiveBank != "inPlace" && this.state.ActiveBank != "Cheque"  ) ?
                                    ...
                                    <div style={{display:'flex',flexDirection:'column'}}>
                                        ...
                                        {this.state.AcceptAddress && this.state.TypeOfPayment == 4 &&
                                            <button className="btn btn-warning YekanBakhFaMedium" style={{  marginBottom: 10 }} onClick={()=>{
                                                this.setState({
                                                    AcceptAddress: false,
                                                    StepNumber: 1,
                                                    chequeEditDisable:false,
                                                    lastPrice:this.state.orgLastPrice
                                                })
                                            }}>اصلاح </button>
                                        }
                ...
                {this.state.CatId && !this.state.loading &&
                    <div  className={this.state.Theme == "2" ? "B-container" : "A-container"} style={{ direction: 'rtl',marginTop: 90 }} >
