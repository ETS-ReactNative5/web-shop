          ...
          <div className="col-lg-12 " >
            <div className="row" style={{ margin: 20 }}>
              ...
              <div className="col-lg-7 col-12 yekan" style={{ textAlign: "right",backgroundColor:'#f5f5f54a',padding:10,borderRadius:10 }}>
                <p className="yekan" style={{fontSize:14,color:'blue'}}>{(car.request && car.request[0]) ? car.request[0].title : car.title}</p>
                <p className="yekan" style={{fontSize:17,marginTop:20}}>{(car.request && car.request[0]) ? car.request[0].desc : car.desc}</p>
                <p>
                  {(car.request && car.request[0]) &&
                  <div style={{color:'#b3b3b3'}}>
                      <span> این درخواست توسط </span> 
                      <span>{car.request[0].OriginalSender}</span>
                      <span> در تاریخ </span>
                      <span>{car.request[0].Date}</span>
                      <span> و در زمان </span>
                      <span>{car.request[0].Time}</span>
              ...
              <div className="col-lg-3 col-12 yekan" style={{ textAlign: "right" }}>
                ...
                <p className="yekan"style={{fontWeight:'bold'}} >فرستنده : {car.SenderName}</p>
                <p className="yekan" style={{color:'#34d634',display:'none'}} >گیرنده : {car.RecieverName}</p>
                {((car.request && car.request[0]) && car.request[0].Priority=="آنی" || car.Priority=="آنی") ?
                  <p className="yekan" style={{color:'red'}} >اولویت : {(car.request && car.request[0]) ? car.request[0].Priority : car.Priority}</p>
                :
                <p className="yekan" >اولویت : {(car.request && car.request[0]) ? car.request[0].Priority : car.Priority}</p>

                }
                <p className="yekan" >{car.Time ? car.Time :  (car.request && car.request[0] && car.request[0].Time) } : {car.Date ? car.Date : (car.request && car.request[0]) && car.request[0].Date}</p>
              ...
              <div className="col-lg-2 col-12 yekan" style={{ textAlign: "center" }}>
                ...
                :
                <button className="btn btn-info yekan" onClick={() => {this.GetAnswer(car) }} style={{ marginTop: "5px", marginBottom: "5px" }}>مشاهده و پاسخ</button>
  ...
  render() {
    ...
    return (
      ...
      <div style={{ direction: 'rtl' }}>
        ...
        <Dialog style={{ width: '60vw' }}  header={this.state.activeIndex == 1 ? "مشاهده" : "مشاهده و پاسخ"}  visible={this.state.visibleManageAction} footer={footer}  onHide={this.onHideFormsDialog} maximizable={true} maximized={true}>
          <div>
            { 
                this.state.request_details && this.state.request_details.map((item,index) => {
                    let header = <div style={{display:'flex',justifyContent:'center',fontSize:13}}><span style={{paddingLeft:10}}>{item.userData[0].name}</span><span style={{paddingLeft:10}}>{item.Date}</span><span style={{paddingLeft:10}}>{item.Time}</span></div>
                    if(!item.draft){
                      return(
                        ...
                        <Panel header={header} style={{marginTop:10}} >
                            ...
                            <p style={{textAlign:'right',fontSize:15,whiteSpace:'pre-wrap'}} className="yekan">{item.answer}</p>
                            {item.attach &&
            ...
            {this.state.activeIndex == 0 && 
            <Panel header={<p>پاسخ درخواست</p>} style={{marginTop:10}} toggleable={true} collapsed={false}>
