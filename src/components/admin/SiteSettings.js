    ...
    axios.post(this.state.url + 'uploadFile', formData, config)
      .then((response) => {
        ...
        if (name == "SpecialImage") {
          this.setState({
            SpecialImage: this.state.absoluteUrl + response.data.split("public")[1]
          })
        }
      ...
      <div style={{ direction: 'rtl' }}>
        ...
        <div className="row justify-content-center">
          ...
          <div className="col-12" style={{ marginTop: 20, background: '#fff' }}>
            <Panel header="تنظیمات کلی سایت" style={{ marginTop: 20, textAlign: 'right', marginBottom: 50, fontFamily: 'yekan' }}>
              <div className="row" >
                <div className="col-12" style={{ display: 'flex', alignItems: 'baseline' }}>
                  ...
                  <label style={{ paddingRight: 5, marginTop: 5 }}>استفاده از امکانات چند فروشگاهی</label>

                </div>
                <div className="col-12" style={{ display: 'flex', alignItems: 'baseline', marginTop: 20 }}>
                  <select className="custom-select YekanBakhFaBold" value={this.state.TypeOfSite} name="TypeOfSite" onChange={(event) => this.setState({ TypeOfSite: event.target.value })} >
                    <option value="">زمینه فعالیت سایت را مشخص کنید</option>
                    <option value="1">محصولات متنوع</option>
                    <option value="2">کالای دیجیتال</option>
                    <option value="3">لوازم منزل ، دکوراسیون</option>
                    <option value="4">محصولات سوپرمارکتی</option>
                    <option value="5">رستوران</option>
                    <option value="6">کتاب ، لوازم التحریر</option>
                    <option value="7">ابزارآلات</option>
                    <option value="8">کیک و شیرینی</option>
                    <option value="9">پوشاک</option>
...
export default withRouter(
  connect(mapStateToProps)(SiteSettings)
);
