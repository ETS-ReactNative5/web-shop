...
class Sales_Registered extends React.Component {
  ...
  onRowSelect(event) {
    ...
    setTimeout(function(){
      ...
      that.setState({
        AllowChangeStatusReadyToSend: selectedP.LastChangeDate_num && ((Date.now() - selectedP.LastChangeDate_num) > 1200000 && selectedP.status == "3") ? false : true
  ...
  shareAddress(p){
    ...
    this.Server.send("AdminApi/getuserInformation", {map:'payk'}, function (response) {
      ...
      let factor="";
      for(let i=0;i<p.products.length;i++){
        factor+=p.products[i].title +"(تعداد : " + p.products[i].number  + ")"+"\n";
      }
      
      if(response.data.result.length > 1 ){
        that.setState({
          SelectPayk:true,
          paykList:{data:response.data.result,address:address,name:p.userData[0].name||"",username:p.userData[0].username,sellername:sellername,factor:factor}
        })
      }else{
        if(response.data.result.length == 1)
          that.sendSms(address,p.userData[0].name,p.userData[0].username,response.data.result[0].username,sellername,factor);
        else
  ...
  sendSms(address,name,username,paykMob,sellername,factor){
      ...
      <div style={{ direction: 'rtl' }}>
          ...
          <div className="col-12" style={{ marginTop: 20, backgroundColor: '#fff' }}>
            ...
            <div className="datatable-responsive-demo">
              <DataTable  resizableColumns={true} paginator={true} className="p-datatable-responsive-demo" rows={10} value={this.state.GridDataFactors} selectionMode="single" selection={this.state.selectedFactor} onSelectionChange={e => { if (e.originalEvent.target.tagName != "I") this.selectedFactorChange(e.value) }} >
                ...
                <Column field="share_address" filter={false} header="اشتراک آدرس"  className="yekan"  />
        ...
        <Dialog header="انتخاب پیک" visible={this.state.SelectPayk}  onHide={this.onHide2} maximizable={false} maximized={false}>
          <div className="row">
            ...
            <div className="col-12">
              {this.state.paykList &&
            <button className="btn btn-success YekanBakhFaMedium" disabled={!this.state.paykMob} style={{marginTop:40,marginBottom:10,width:'100%'}} onClick={()=>this.sendSms(this.state.paykList.address,this.state.paykList.name,this.state.paykList.username,this.state.paykMob,this.state.paykList.sellername,this.state.paykList.factor)}>اشتراک گذاری اطلاعات کاربر</button>
              }
            </div>
          </div>
        </Dialog>
        <Dialog header="جزئیات فاکتور" visible={this.state.selectedFactor} style={{ width: '80vw' }} minY={70} onHide={this.onHide} maximizable={false} maximized={true}>
          <div style={{ overflowY: 'auto', overflowX: 'hidden', minHeight: 400 }}>
            ...
            <DataTable onRowSelect={this.onRowSelect} className="p-datatable-responsive-demo" responsive selection={this.state.selectedProduct1} onSelectionChange={e => {
              ...
              {this.state.isMainShop == 1 ?
              <Column field="number" body={ProductBodyTemplate} edi_tor={(props) => this.gridEditor('number', props)} header="تعداد" className="yekan" style={{ textAlign: "right" }} />
                :
