...
class TalaVam extends React.Component {
  constructor(props) {
    super(props);
  ...
  GetRecords(Filter) {
    ...
    this.Server.send("AdminApi/GetTalaVams", param, SCallBack, ECallBack)
  }
  EditRecord(){
    let that = this;
    ...
    if(this.state.SelectedStatus == "-1")
      msg = "درخواست وام طلا شما به دلیل زیر توسط سیستم رد شده است" + "\n" + this.state.selectedDescription  + "\n" + "قرض الحسنه انصارالهدی" ;
    if(this.state.SelectedStatus == "1")
      msg = "وام طلای درخواستی شما تایید شده است " + "\n" + "زمان مراجعه به صندوق انصارالهدی جهت تکمیل مراحل : " + "\n" + this.state.selectedDescription  ;
    that.setState({
        loading: 1
    })
    ...
    this.Server.send("AdminApi/EditTalaVam", param, SCallBack, ECallBack)
  ...
  render() {
    ...
    return (
      ...
      <div style={{ direction: 'rtl' }}>
        ...
        <p className="mt-5" style={{textAlign:'center',fontSize:30}}>درخواست های وام طلا</p>
...
export default withRouter(
  connect(mapStateToProps)(TalaVam)
);
