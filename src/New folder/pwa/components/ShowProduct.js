			...
			<div>
				<div className="row">
					<div className="col-12" style={{ textAlign: 'right', marginTop: 15 }}>
						...
						{
							((!this.state.NoOff ? parseInt(this.props.off||0) : 0) + this.state.product.off) > "0" &&
							<div className="product_price YekanBakhFaBold oldPrice_product" style={{ direction:'rtl',textAlign: 'center',maxWidth:150 }}><span>{this.persianNumber(this.roundPrice(parseInt(this.state.product.price).toString()).replace(/\B(?=(\d{3})+(?!\d))/g, ","))}</span> <span>تومان</span> </div>
						}
						{this.state.product.number > 0 && this.state.product.price > 0 &&
							<div className="product_price YekanBakhFaBold" style={{ direction:'rtl',marginTop: 50,color:'red', textAlign: 'right', fontSize: 25 }}> <span>{this.persianNumber(this.roundPrice(parseInt(this.state.product.price - ((this.state.product.price * ((!this.state.NoOff ? parseInt(this.props.off||0) : 0) + this.state.product.off)) / 100)).toString()).replace(/\B(?=(\d{3})+(?!\d))/g, ","))}</span> <span>تومان</span></div>

						}

					</div>
					<div className="col-12" style={{ textAlign: 'center' }}>
						<img src={this.state.pic} style={{ maxHeight: 400,width:'100%' }} name="pic" alt="" />
