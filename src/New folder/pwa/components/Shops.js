...
class Shop extends React.Component {
    ...
    getProducts(p) {
        ...
        let SCallBack = function (response) {
            ...
            if (!that.state.ShopIsOpen && p.first) {
                that.toast.current.show({ severity: 'error', summary: 'فروشگاه بسته است', detail: <div className="iranyekanwebmedium">امکان خرید از این فروشگاه وجود ندارد</div>, life: 8000 });
            }
            if (!that.state.InTime && p.first) {
                if ((that.state.Time1 == "00:00" && that.state.Time2 == "00:00") && (that.state.Time3 == "00:00" && that.state.Time4 == "00:00")) {
                    ...
                    that.toast.current.show({
                        severity: 'warn', summary: 'فروشگاه بسته است', detail: <div className="iranyekanwebmedium">ساعت کار امروز فروشگاه <br />
    ...
    SendToCart(product, number) {
        ...
        if (!PeykInfo[2].city || !PeykInfo[2].subCity) {
            that.toast.current.show({ severity: 'warn', summary: 'عدم امکان خرید', detail: <div><span>آدرس خود را ثبت کنید</span><br /><br /><Link to={`${process.env.PUBLIC_URL}/User?ShopId=${that.state.id}`} style={{ textDecoration: 'none', color: '#333' }}>ویرایش آدرس</Link></div>, life: 8000 });
            return;
        }
        ...
        if ((userLocation == 4 && !PeykInfo[0].SendToCountry) || (userLocation == 3 && !PeykInfo[0].SendToState) || (userLocation == 2 && !PeykInfo[0].SendToNearCity) || (userLocation == 1 && !PeykInfo[0].SendToCity)) {
            ...
            that.toast.current.show({ severity: 'warn', summary: 'عدم امکان خرید', detail: <div><span>امکان خرید این محصول با توجه به آدرس محل سکونت شما وجود ندارد</span><br /><br /><Link to={`${process.env.PUBLIC_URL}/User?ShopId=${that.state.id}`} style={{ textDecoration: 'none', color: '#333' }}>ویرایش آدرس</Link></div>, life: 8000 });
        ...
        let param = {
            ...
            Number: number || 1,
            UId: that.state.UId,
            Price: (this.roundPrice(product.price - ((product.price * ((!that.state.NoOff ? parseInt(that.props.off||0) : 0) + product.off)) / 100))),
            Status: "0",
            Type: "insert",
            token: localStorage.getItem("api_token"),
                ...
                <div className="col-12 col-lg-3 col-md-4 col-sm-6" style={{ textAlign: 'center', paddingRight: 0, paddingLeft: 0, paddingTop: 0, paddingBottom: 0 }}>
                    <div className="product-grid-item card" style={{ padding: 10, minHeight: 400 }} onClick={() => {
                        ...
                        {!this.state.ProductBase && car.number > 0 && this.state.ShopIsOpen && this.state.InTime &&
                            <span className="fa fa-plus-circle text-success" onClick={(e) => { this.openSideBar(car); e.stopPropagation(); }} style={{ fontSize: 30, position: 'absolute', top: 5, right: 5, cursor: 'pointer', zIndex: 5,color:'#2cc73c' }}></span>
                        }
                                ...
                                <div style={{position:'relative'}}>
                                    {car.number > 0 ?
                                        <div style={{ textAlign: 'left' }}>
                                            ...
                                            <div className="product-price YekanBakhFaMedium" style={{ fontSize: 20 }} >{this.persianNumber(this.roundPrice((car.price - (car.price * ((!car.NoOff ? parseInt(this.props.off||0) : 0) + car.off)) / 100).toString()).replace(/\B(?=(\d{3})+(?!\d))/g, ","))} تومان</div>
                        ...
                        <div style={{ position: 'absolute', bottom: 15, width: '100%', left: 0 }}>
                            {this.state.ProductBase &&
                                <Link className="p-button-info btn-warning " to={`${process.env.PUBLIC_URL}/products?id=${(car.product_detail && car.product_detail[0]) ? car.product_detail[0]._id : car._id}`} href="#" style={{ padding: 10, marginTop: 10, width: '85%', fontFamily: 'YekanBakhFaMedium' }}>
                                    مشاهده جزئیات / خرید
                                </Link>
                            }
                        </div>
    ...
    render() {
        ...
        if (this.state.goToLogin) {
            return <Redirect to={"/User?ShopId="+this.state.id+""} />;
        }
        return (
            ...
            <div>
                            ...
                            <div style={{ position: 'relative'}}>
                                <div>
                                    {this.state.shop.SpecialPic ?
                                        <img src={this.state.absoluteUrl + this.state.shop.SpecialPic.split("public")[1]} style={{  marginBottom: 50, maxHeight: 450, width: '100%' }} />
                                        :
                                        <img src="./bg-seller.png" style={{ marginBottom: 30, marginBottom: 50, maxHeight: 450, width: '100%' }} />
                                    ...
                                    <div style={{ position: 'absolute', display: 'flex', bottom: 0, alignItems: 'center', backgroundColor: '#fff', width: '100%', height: 50, justifyContent: 'space-between' }} >
                                        {this.state.shop.logo &&
                                            <img src={this.state.absoluteUrl + this.state.shop.logo.split("public")[1]} className="d-sm-block d-none" style={{ position: 'absolute', top: -5, width: 70, height: 70, right: 20,borderRadius:5 }} />
                                        }
                                        <div className="yekan" style={{ width: 30 }}>

                                        </div>

                                        <div className="yekan" style={{ fontSize: 16, padding: 5,fontSize:11 }}>
                                        ...
                                        <div className="col-12" >
                                            {
                                                this.state.shop &&
                                                <div>
                                                    <p style={{ fontSize: 26, textAlign: 'center', marginTop: 0, color: '#fff', backgroundColor: '#00bfd6'}} className="yekan">
                                                       {this.state.shop.name}
                                                    </p>
                                                    <p dangerouslySetInnerHTML={{ __html: this.state.shop.about }} style={{ display: 'none' }}>

                                                    </p>
                                                    <div style={{textAlign:'right',maginTop:10,marginBottom:10,display:'none'}}>
                                                    ...
                                                    <div style={{ marginTop: 10, border: '1px solid #eee', borderRadius: 5, padding: 5, textAlign: 'right' }} >
                                                        <p className="yekan" style={{padding:0}}>مرتب سازی بر اساس : </p>
                                                        <SelectButton optionLabel="name" optionValue="value" style={{ textAlign: 'right' }} value={this.state.Sort} options={this.state.SortOptions} onChange={(e) => {
                                                    ...
                                                    <ListBox value={this.state.cat} style={{ borderColor: '#ececec',display:'none', textAlign: 'right', fontFamily: 'YekanBakhFaMedium', marginTop: 50,maxHeight:400,overflow:'auto' }} options={this.state.cats} onChange={(e) => { this.changeCatList(e) }} />
                ...
                <Dialog visible={this.state.selectedProduct} onHide={this.onHide}  maximizable={false} maximized={true}>
                    <ShowProduct data={this.state.curentProduct} />
                </Dialog>
                <Dialog header="عدم امکان خرید" visible={this.state.notAllow} position='top' maximized={true}  onHide={() => this.setState({ notAllow: false })}
                ...
                <Sidebar visible={this.state.visibleSideBar} onHide={() => this.setState({ visibleSideBar: false })}>
                    ...
                    {this.state.CartItems.length > 0 &&
                        <div style={{maxHeight:400,overflow:'auto'}}>
                            <div style={{ textAlign: 'center' }}>
                                <i className="fal fa-shopping-cart mr-4 mr-1 " style={{ fontSize: 50, marginTop: 20, marginBottom: 20,color:'red' }} /><br />
                            </div>
                                ...
                                return (
                                    <div className="row" style={{ border: '1px solid #eee', marginBottom: 10 }}>
                                        ...
                                        <div className="col-12" style={{ textAlign: 'right', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#eee' }}>
                                            <span className="fa fa-minus-circle text-danger" onClick={(e) => { this.changeCart(parseInt(item.number) - 1, item.product_detail_id || item.product_id); }} style={{ fontSize: 20, cursor: 'pointer',color:'red' }}></span>
                                            <div><span className="iranyekanwebmedium">تعداد : </span><label className="iranyekanwebmedium">{item.number}</label></div>
                                            <span className="fa fa-plus-circle" onClick={(e) => { this.changeCart(parseInt(item.number) + 1, item.product_detail_id || item.product_id); }} style={{ fontSize: 20, cursor: 'pointer',color:'green' }}></span>
                    ...
                    {this.state.AllowSale && this.state.CartItems && this.state.CartItems.length > 0 ?
                        <div style={{ textAlign: 'center', marginTop: 50 }}>
                            <Link to={`${process.env.PUBLIC_URL}/cart`} className="iranyekanwebmedium" style={{background:'#d6dad6',padding:10,borderRadius:5}}>ادامه فرآیند خرید</Link>
                        </div>
                        :
