		...
		that.Server.send("AdminApi/getSettings", {}, function (response) {
			...
			if (response.data.result) {
				that.setState({
					...
					InRaymand: response.data.result[0] ? response.data.result[0].Raymand : false,
					Theme: response.data.result[0] ? response.data.result[0].Theme : "1",
					credit: (response.data.result[0].Raymand && that.state.credit) ? parseInt(that.state.credit.toString().substr(0, that.state.credit.toString().length - 1)) : parseInt(that.state.credit)
			...
			this.state.Theme != null ?
				<div style={{ background: '#fff', paddingBottom: 10 }} >
					<OverlayPanel ref={this.op} showCloseIcon dismissable>
						...
						{(this.state.credit != "undefined" && this.state.credit != 0 && this.state.credit != undefined) &&
							<p className="yekan" style={{ textAlign: 'right' }} ><span>موجودی کیف پول : </span><span>{this.state.credit ? this.persianNumber(this.state.credit.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : 0} تومان</span></p>
						}
					...
					{this.state.top_image ?
						...
						:
						<div style={{ padding: 5, backgroundSize: 'cover', backgroundRepeat: 'no-repeat', backgroundPosition: '50%' }} ></div>
					}
