...
class Off_List extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.SetOffs = this.SetOffs.bind(this);
  ...
  SetOffs() {
    ...
    let param = {
      ...
      all: this.state.all,
      Amount: this.state.Amount.toString().replace(/,/g, ""),
    ...
    this.Server.send("AdminApi/setOffList", param, SCallBack, ECallBack)
  }
  CreateForm() {
    this.setState({
  ...
  selectedComponentChange(value) {
    ...
    this.setState({
      ...
      all: value.all,
      enable: value.enable,
      Amount: value.Amount.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ","),
      score: value.score,
  ...
  getOffsList() {
    ...
    this.Server.send("AdminApi/getOffsList", param, SCallBack, ECallBack)
  }
  delField(rowData) {
    this.setState({
      visibleManageField:false
...
itemTemplate(brand) {
  ...
  render() {
    const footer = (
      <div>
        <button className="btn btn-primary irsans" onClick={this.SetOffs} style={{ width: "200px", marginTop: "20px", marginBottom: "20px" }}> اعمال </button>
            ...
            <div className="row" style={{alignItems:'baseline'}}>
              ...
              <div className="group" >
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Checkbox inputId="active" value={this.state.all} checked={this.state.all} onChange={e => this.setState({ all: e.checked })}></Checkbox>
                        <label htmlFor="active" className="p-checkbox-label yekan" style={{ paddingRight: 5,marginBottom:0 }}>همه ی کاربران</label>
                      </div>
              </div>
              </div>
...
export default withRouter(
  connect(mapStateToProps)(Off_List)
);
